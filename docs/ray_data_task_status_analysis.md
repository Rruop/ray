...
根据 `src/ray/protobuf/common.proto` 中的定义：

```protobuf
enum TaskStatus {
  NIL = 0;                                    // 没有状态信息
  PENDING_ARGS_AVAIL = 1;                     // 等待依赖数据创建完成
  PENDING_NODE_ASSIGNMENT = 2;                // 等待调度器分配节点
  PENDING_OBJ_STORE_MEM_AVAIL = 3;            // 等待 Object Store 内存释放
  PENDING_ARGS_FETCH = 4;                     // 正在下载依赖数据到节点
  SUBMITTED_TO_WORKER = 5;                    // 已提交给 Worker，在队列中等待
  PENDING_ACTOR_TASK_ARGS_FETCH = 6;          // Actor Task 正在获取参数
  PENDING_ACTOR_TASK_ORDERING_OR_CONCURRENCY = 7;  // Actor Task 等待顺序/并发约束
  RUNNING = 8;                                // 正在运行
  RUNNING_IN_RAY_GET = 9;                     // 运行中但阻塞在 ray.get()
  RUNNING_IN_RAY_WAIT = 10;                   // 运行中但阻塞在 ray.wait()
  FINISHED = 11;                              // 完成
  FAILED = 12;                                // 失败
  GETTING_AND_PINNING_ARGS = 13;              // 正在获取和固定参数
}
```
...
│        ↑                        ↑                          ↑                    ↑
│   等待上游数据              调度器选节点               已发送到Worker          正在执行
│                                 │                          │
│                                 ↓                          ↓
│                    PENDING_OBJ_STORE_MEM_AVAIL    (Worker任务队列中等待)
│                    PENDING_ARGS_FETCH
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────────────────────┐
│                                                                                 │
│  PENDING_ARGS_AVAIL ──→ SUBMITTED_TO_WORKER ──→ PENDING_ACTOR_TASK_* ──→ RUNNING
│        ↑                        ↑                        ↑
│   等待参数就绪             发送到Actor所在Worker    等待Actor内部调度
│                                                          │
│                                        ┌─────────────────┴─────────────────┐
│                                        ↓                                   ↓
