...
def extract_pipeline_metrics(ds, num_gpus=0, outdir=None):
    ...
    ray_wait_s = summary.streaming_exec_ray_wait_s
    if ray_wait_s:
        metrics["ray_wait_s"] = round(ray_wait_s, 2)

    on_data_ready_s = summary.streaming_exec_on_data_ready_s
    if on_data_ready_s:
        metrics["on_data_ready_s"] = round(on_data_ready_s, 2)

    dispatch_s = summary.streaming_exec_dispatch_s
    if dispatch_s:
